...
class LibraryService:
    ...
    async def ingest_upload(self, upload: UploadFile, metadata: FileMetadata) -> LibraryFile:
        ...
        source_title = metadata.source_title or Path(upload.filename or "未命名资料").stem
        file = LibraryFile(
            id=str(uuid4()),
            sha256=sha256,
        ...
        return self.db.insert_library_file(file)
    ...
    def list_files(
        ...
        return self.db.list_library_files(subject, category, source_type, search)
    ...
    def update_file(self, file_id: str, patch: LibraryFilePatch) -> LibraryFile:
        if patch.subject is not None:
            patch = patch.model_copy(update={"subject": self._normalize_subject_or_422(patch.subject)})
    ...
    async def upload_to_context_source(
        ...
        source_title = metadata.source_title or Path(filename).stem
    ...
    async def preview(self, file_id: str, max_chars: int = 4000) -> dict:
        file = self.get_file(file_id)
        text = await self.parse_and_cache(file)
        return {
            "file_id": file.id,
    ...
    def _normalize_metadata(self, metadata: FileMetadata) -> FileMetadata:
        return metadata.model_copy(update={"subject": self._normalize_subject_or_422(metadata.subject)})
